...
a `.txt` file with the same base name is used as the app description. It can be short or long and can contain multiple lines.

for APKs larger than GitHub's file upload limit, use a `.url` file instead. Put the direct APK URL inside it:
...
The contents of `mega-app.url` should be just the download URL. The store will use that URL as the download button.
