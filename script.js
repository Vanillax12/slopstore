...
function fileDownloadUrl(owner, repo, file) {
  return file.download_url || rawUrl(owner, repo, file.path);
}
...
async function getDescription(owner, repo, txtFile) {
  ...
  const response = await fetch(fileDownloadUrl(owner, repo, txtFile), {
    cache: "no-store"
  });
...
async function makeAppList(files, owner, repo) {
  ...
  return installableFiles.map(file => {
    ...
    return {
      ...
      iconUrl: icon ? fileDownloadUrl(owner, repo, icon) : null,
      apkUrl: isUrl ? null : fileDownloadUrl(owner, repo, file),
...
async function resolveAppFiles(apps, files, owner, repo) {
  for (const app of apps) {
    ...
    if (descriptionFile) {
      ...
      try {
        const response = await fetch(fileDownloadUrl(owner, repo, descriptionFile));
    ...
    if (app.urlFile) {
      try {
        const response = await fetch(app.urlFile.download_url || rawUrl(owner, repo, app.urlFile.path));
